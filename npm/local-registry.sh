...
PORT="${CS_LEDGER_REGISTRY_PORT:-4873}"
...
# dependencies: it is a thing you run, not a thing cs-ledger is built from, and
...
version="$(node -p "require('./npm/dist/cs-ledger/package.json').version")"
...
echo "==> publishing"
NPM_CONFIG_USERCONFIG="$NPMRC" CS_LEDGER_NPM_TAG=dev ./npm/publish.sh > "$STATE/publish.log" 2>&1 ||
  { echo "the publish failed; see $STATE/publish.log" >&2; exit 1; }

cat <<EOF

Published @codesweep-ai/cs-ledger@$version to the registry on this machine.

  Browse   $URL/-/web/detail/@codesweep-ai/cs-ledger
  Install  NPM_CONFIG_USERCONFIG=$PWD/$NPMRC npm install @codesweep-ai/cs-ledger@dev
