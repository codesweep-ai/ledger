...
// then its hash — and `cs-ledger version` prints it. Reusing it means the package
...
function devVersion() {
  ...
  if (!m) fail(`could not read a version out of \`cs-ledger version\`, which printed: ${out.trim()}`);
...
function version() {
  const override = process.env.CS_LEDGER_NPM_VERSION;
  ...
  if (!/^\d+\.\d+\.\d+(-[0-9A-Za-z.-]+)?(\+[0-9A-Za-z.-]+)?$/.test(raw)) {
    fail(
      `"${raw}" is not a version npm will accept.\n` +
        ...
        `  CS_LEDGER_NPM_VERSION=0.1.0-snapshot.1 node npm/build.mjs`,
    );
  }
  return raw;
}
...
const repository = {
  type: "git",
  url: "git+https://github.com/codesweep-ai/ledger.git",
};
...
for (const [target, { os, cpu, suffix }] of Object.entries(PLATFORMS)) {
  const name = `@codesweep-ai/cs-ledger-${suffix}`;
  ...
  writeJSON(join(dir, "package.json"), {
    name,
    version: VERSION,
    description: `The cs-ledger binary for ${os} ${cpu}. Installed by @codesweep-ai/cs-ledger; not used directly.`,
    license: "Apache-2.0",
    repository,
    homepage: "https://github.com/codesweep-ai/ledger#readme",
    ...
    // No `exports`, on purpose: the wrapper resolves `<name>/bin/cs-ledger`, and
  ...
  writeFileSync(
    join(dir, "README.md"),
    `# ${name}\n\n` +
      `The cs-ledger binary for ${os} ${cpu}.\n\n` +
      `This package is one of four, and installing it directly is not the way in. ` +
      `Install [\`@codesweep-ai/cs-ledger\`](https://www.npmjs.com/package/@codesweep-ai/cs-ledger), ` +
...
console.log(`cs-ledger ${VERSION} -> npm/dist/`);
for (const name of [...platformNames, "@codesweep-ai/cs-ledger"].sort()) {
  console.log(`  ${name}`);
}
console.log(`\nPublish with:\n  npm/publish.sh`);
