...
const PACKAGES = {
  "darwin arm64": "@codesweep-ai/cs-ledger-darwin-arm64",
  "darwin x64": "@codesweep-ai/cs-ledger-darwin-x64",
  "linux arm64": "@codesweep-ai/cs-ledger-linux-arm64",
  "linux x64": "@codesweep-ai/cs-ledger-linux-x64",
};
...
export class UnsupportedPlatformError extends Error {
  constructor(platform, arch) {
    super(
      `cs-ledger has no build for ${platform}/${arch}.\n` +
        `Built platforms: ${Object.keys(PACKAGES).join(", ").replace(/ (?=\w+,|\w+$)/g, "/")}.\n` +
        `Ask for one at https://github.com/codesweep-ai/ledger/issues, or build from ` +
        `a clone: https://github.com/codesweep-ai/ledger/blob/main/INSTALL.md`,
    );
...
export class MissingBinaryError extends Error {
  constructor(pkg, cause) {
    super(
      `cs-ledger is installed, but ${pkg} is not.\n` +
        `That package carries the binary for this platform and installs with ` +
        `cs-ledger itself, so this usually means a node_modules built on another ` +
...
// The absolute path to the cs-ledger binary for this machine.
//
// CS_LEDGER_BINARY overrides the lookup entirely. That is what lets this
...
export function binaryPath() {
  const override = process.env.CS_LEDGER_BINARY;
  ...
  // pnpm or Yarn.
  try {
    return require.resolve(`${pkg}/bin/cs-ledger`);
  } catch (cause) {
