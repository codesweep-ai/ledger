...
A release takes the `latest` tag. Every commit on `main` that passes CI also goes out under the
`dev` tag, versioned by the commit it came from. A project pins that when it wants a fix before
the next release:
